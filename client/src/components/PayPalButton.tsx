...
export default function PayPalButton({ amount, currency, intent }: PayPalButtonProps) {
  ...
  const onCancel = async (data: any) => console.log("onCancel", data);
  const onError = async (data: any) => console.log("onError", data);
  ...
  const initPayPal = async () => {
    try {
      ...
      return () => {
        if (paypalButton) paypalButton.removeEventListener("click", onClick);
      };
    } catch (e) {
      console.error(e);
    }
